...
class distribution_manager(): 
    ...
    def load_timeseries(self):
        ...
        if data_type == 'simulation':
            ...
            self.data_table = t
            self.description = ric + ' Distribution \n' + '\n' + 'Santiago Huerdo | @santiagohuerdo'
...
class capm_manager():
    ...
    def plot_timeseries(self):
        ...
        plt.title('Time series of prices \n' + '\n' + 'Ticker: ' + self.security + ' | Benchmark: ' + self.benchmark + '\n' + \
                  '\n Santiago Huerdo | @santiagohuerdo')
        plt.xlabel('Time')
        plt.ylabel('Prices')
        ax = plt.gca()
        ax1 = self.data_table.plot(kind='line', x='date', y='price_x', ax=ax, grid=False,\
                                  color='blue', label=self.benchmark)
        ax2 = self.data_table.plot(kind='line', x='date', y='price_y', ax=ax, grid=False,\
    ...
    def plot_linear_regression(self):
        x = self.data_table['return_x'].values
        y = self.data_table['return_y'].values
        str_title = 'Scatterplot of returns | ' + 'Linear regression' + '\n' + '\n' + 'Ticker: ' + self.security\
            + ' | Benchmark: ' + self.benchmark + '\n' + '\n' + 'Santiago Huerdo | @santiagohuerdo'
        plt.figure(figsize=(12,8))
        plt.title(str_title)
        plt.scatter(x,y)
        plt.plot(x, self.predictor_linreg, color='red')
